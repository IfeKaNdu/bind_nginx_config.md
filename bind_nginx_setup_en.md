...
```
Find descriptions of the above networking variables  in your `/usr/share/doc/initscripts*/sysconfig.txt` file.

### Assign A FQDN (Fully Qualified Domain Name) For The Server

Edit the `/etc/sysconfig/network` file to add the HOSTNAME of your choice. Mine is `odii.cic.com`
```bash
...
```
Notice in the above,  `192.168.8.2`  and  `odii.cic.com` was added which will  change depending on the static IP and HOSTNAME of your choice.
`127.0.0.1   localhost localhost.localdomain localhost4 localhost4.localdomain4
::1         localhost localhost.localdomain localhost6 localhost6.localdomain6 `
...
Thd forward zone is `cic.com` and the reverse zone is `8.168.192` which is the inverse of the network ID of the assigned IP address `192.168.8`
...
@	IN SOA	odii.cic.com.  root.odii.cic.com. (
					...
					3H )	; minimum
	IN NS	odii.cic.com.
2	IN PTR	odii.cic.com.		# the 2 means the host id of the assigned IP address
```
...
```
To check whether your connection is working run the `dig` and `host` commands which will dig for the named service.
```bash
$ dig odii.cic.com
...
;; MSG SIZE  rcvd: 71
```
and also it can be verified using the `host` command
```bash
$ host odii.cic.com
